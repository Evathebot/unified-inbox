...
export class ContactResolver {
  ...
  private async addIdentity(contactId: string, participant: ParticipantInfo): Promise<void> {
    // findFirst + update/create avoids relying on the 3-column unique key
    // (contactId+platform+platformUserId) and correctly handles re-linking an
    // identity to a different contact when merging.
  ...
  private async createContact(participant: ParticipantInfo): Promise<string> {
    ...
    // Use findFirst + update/create to prevent duplicate identities without
    // relying on the 3-column unique key (which requires contactId upfront).
    const existingIdentity = await this.prisma.contactIdentity.findFirst({
      where: {
        platform: participant.platform,
        platformUserId: participant.externalId,
