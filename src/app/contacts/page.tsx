      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="backdrop-blur-xl bg-white/[0.06] border border-white/[0.08] rounded-xl p-5">
            ...
            <p className="text-3xl font-bold text-white">{mockContacts.length}</p>
          </div>
          <div className="backdrop-blur-xl bg-white/[0.06] border border-white/[0.08] rounded-xl p-5">
            <p className="text-gray-400 text-sm mb-1">Avg Relationship Score</p>
            <p className="text-3xl font-bold text-white">
              {Math.round(mockContacts.reduce((acc, c) => acc + c.relationshipScore, 0) / mockContacts.length)}%
            </p>
          </div>
          <div className="backdrop-blur-xl bg-white/[0.06] border border-white/[0.08] rounded-xl p-5">
            <p className="text-gray-400 text-sm mb-1">Active Today</p>
            <p className="text-3xl font-bold text-white">
              {mockContacts.filter(c => Date.now() - c.lastInteraction.getTime() < 24 * 60 * 60 * 1000).length}
            </p>
          </div>
        </div>
